...
% %读取训练数据
train_label=csvread('E:\traffic_data\worked_data\workday_1traffic.csv',2,2,[2 2 309 2]);
train_data=csvread('E:\traffic_data\worked_data\workday_1traffic.csv',1,1,[1 1 308 2]);
train_len = length(train_label)

% %读取测试数据
test_label=csvread('E:\traffic_data\worked_data\workday_2traffic.csv',2,2,[2 2 206 2]);
test_data=csvread('E:\traffic_data\worked_data\workday_2traffic.csv',1,1,[1 1 205 2]);
...
X={170,340,510,680,850,1020,1190,1360,1530,1700,1870,2040,2210,2380,2550,2720,2890,3060};

a = 1:1:308;
figure(1)
plot(a,train_label,'r-*',a,predict_1,'b:o')
set(gca,'xtick',17:17:18*17)
...
%      X{i-6} = num2str(i);
% end
X={60,120,240,360,480,600,720,840,960,1080,1200,1320,1440,1560,1680,1800,1920,2040};
a = 1:1:205;
figure(2)
plot(a,test_label,'r-*',a,predict_2,'b:o')
set(gca,'xtick',12:12:18*12)
set(gca,'xticklabel',X)
legend('真实值','预测值')
xlabel('时  间(min)')
ylabel('车流量(辆/10min)')
